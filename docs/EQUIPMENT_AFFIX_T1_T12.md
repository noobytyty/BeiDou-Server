...
武器词缀生成按武器类别优先应用职业适配，而不是只依赖 WZ 的 `reqJob`。剑/斧/钝器/枪/矛走战士路线，短杖/长杖走法师路线，弓/弩走弓手路线，短刀/拳套走飞侠路线，指虎/手枪走海盗路线；因此即使 WZ 的 `reqJob` 为 0，也不会生成明显跨职业的主属性或攻击属性词缀。历史装备不重抽词缀，仍保持兼容。

武器上的属性类词缀进一步限制为该路线的主属性和副属性：战士 STR/DEX，法师 INT/LUK，弓手 DEX/STR，飞侠 LUK/DEX，海盗 STR/DEX；只保留这些属性的单属性、双属性和对应的职业攻击组合（如 `STR_WATK`、`DEX_WATK`、`INT_MATK`），会排除 `STR_MP`、`DEX_HP`、`INT_MDEF`、`ALL_STAT` 等混入其他维度的组合。
...
带词缀装备分解时，在原版制作结晶返还之外，按词缀效果追加现有的 8 种魔法粉末。粉末按效果类别映射，不新增客户端道具：

| 粉末 | 词缀类别 |
| --- | --- |
| 褐色 `4007000` | 基础属性、全属性及其他未分类属性 |
| 白色 `4007001` | WATK、MATK 和职业攻击组合 |
| 蓝色 `4007002` | HP、MP 和资源组合 |
| 绿色 `4007003` | WDEF、MDEF 和防御组合 |
| 黄色 `4007004` | ACC、AVOID 和命中防护组合 |
| 紫色 `4007005` | SPEED、JUMP 和移动组合 |
| 红色 `4007006` | Boss、无视防御、掉落、经验、金币等特殊效果 |
| 黑色 `4007007` | 火、冰、雷、圣元素伤害 |

每条词缀独立计算数量：类别基础值（普通属性 1、攻击/资源 2、特殊/元素 3）+ 品质等级折算 `rarity / 2` + T 阶折算 `ceil(T / 3)`，同类粉末在一次分解中合并。无词缀装备继续使用原版分解逻辑。

重铸时按当前装备中所有未锁定词缀计算并消耗对应粉末，数量使用与分解相同的公式；锁定词缀不消耗粉末且会保留。执行重铸前会一次性校验所有粉末库存，材料不足时不会扣除金币或改变装备。
