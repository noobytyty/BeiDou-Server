...
> 说明：本文中的数值和抽取规则对应当前服务端代码及 `V1.11.24`–`V1.11.55` 迁移。词缀展示名称由服务端 i18n 提供，阶级前缀按 `affix_code + affix_tier` 独立配置。

> 2026-09-03：新增 `V1.11.55__rebalance_high_rarity_and_affix_tier_weights.sql`，调整高级品质和 T3-T12 的抽取权重，并让品质对 T5 以上阶级施加轻微次级修正，不修改词条数值范围。
...
需求等级仍然决定词条平均阶级和可抽取窗口；品质只对 T5 以上阶级的最终权重做轻微修正。
品质 0 至 6 对 T12 的倍率约为 `0.92`、`0.96`、`1.00`、`1.04`、`1.08`、`1.12`、
`1.16`，T5-T11 按阶级距离 T4 的步数线性过渡。品质不改变词条数量、数值范围或等级窗口。
...
重铸时按当前装备中所有未锁定词缀计算并消耗对应粉末，数量使用与分解相同的公式；锁定词缀不消耗粉末且会保留。金币基础费用为 50,000，按品质使用 `1.8^品质` 倍率，并按锁定词条使用 `1.6^锁定数量` 倍率；执行重铸前会一次性校验所有粉末库存，材料不足时不会扣除金币或改变装备。分解金币以装备商店价的 15% 作为基础值，再叠加品质和词条阶级倍率，最低为 1,000 金币。
