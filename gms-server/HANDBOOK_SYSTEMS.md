...
- **商城现金武器兼容**：`170xxxx` 现金武器外观通过 `Character.wz/Weapon/017xxxxx.img.xml` 的数字类型节点决定可覆盖的真实武器；描述为所有/任意武器但缺失标准节点时，可用同文件已有动作的 UOL 别名补齐，不新增美术资源。客户端 `Data/Character/Weapon` 必须同步这些 XML 节点。
